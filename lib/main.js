...
function fligner() {
	...
	if ( opts.groups ) {
		...
		if ( ngroups < 2 ) {
			throw new Error( format( '1H38W', 'groups', levels ) );
		}
		for ( i = 0; i < ngroups; i++ ) {
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '1H38V', 'alpha', alpha ) );
	}
	x = [];
	for ( i = 0; i < ngroups; i++ ) {
		if ( !isCollection( args[ i ] ) ) {
			throw new TypeError( format( '1H38X', args[ i ] ) );
		}
		if ( args[ i ].length === 0 ) {
			throw new Error( format( '1H38Y', args[ i ] ) );
		}
		loc = median( args[ i ] );
